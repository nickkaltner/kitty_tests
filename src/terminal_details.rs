...
/**
 * Detects the terminal dimensions in pixels by reading the escape sequence
 *
 * outputs Result(width, height)
 */
pub fn get_terminal_dimensions() -> Result<(u16, u16), String> {
    if !stdin().is_terminal() {
        return Err("This function needs a terminal".to_string());
    }
    //eprintln!("checking terminal capabilities");
    ...
    tcsetattr(stdin_fd, TCSANOW, &old_termios).unwrap(); // reset the stdin to
